...
async function request<T>(
  ...
  if (!API_URL) {
    return {
      ...
      message: 'VITE_APPS_SCRIPT_URL belum dikonfigurasi.'
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return {
        success: false,
        error: 'HTTP_ERROR',
        message: 'Server mengembalikan HTTP ' + response.status
      };
    }
...
export const api = {
  ...
  changePassword: (sessionToken: string, currentPassword: string, newPassword: string) =>
    request<{ changed: boolean; message: string }>('changePassword', {
      sessionToken,
      currentPassword,
      newPassword
    }),
